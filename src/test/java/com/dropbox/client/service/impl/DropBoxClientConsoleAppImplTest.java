...
public class DropBoxClientConsoleAppImplTest {
  ...
  @Mock private DropBoxGateway dropBoxGateway = DropBoxGatewayImpl.getInstance();

  @InjectMocks
  private DropBoxClientConsoleApp dropBoxClientConsoleApp =
  ...
  public void generateAccessToken_when_authorization_failed() throws DbxException {
    when(dropBoxGateway.authorize(anyString(), anyString(), anyString()))
        .thenThrow(new DbxException("authorization call failed"));
  ...
  public void outPutClientFullAccount_when_fetch_client_info_call_failed() {
    when(dropBoxGateway.getClient(anyString(), anyString()))
        .thenThrow(new RuntimeException("invalid accessToken"));
